...
export function TimeSelector({
  ...
  return (
    <div className="flex w-[12rem] flex-shrink flex-col items-center gap-4">
      {availableTimes.map((dt) => (
        <button
          key={dt.toISOString()}
          disabled={selectedTime === dt}
          onClick={() => setSelectedTime(dt)}
          className="w-full rounded-md border-2 border-accent border-opacity-25 p-4 text-center font-semibold text-accent transition-all duration-200 hover:bg-accent/10 disabled:bg-accent/15"
        >
          {format(dt, "HH:mm aaa")}
        </button>
      ))}
